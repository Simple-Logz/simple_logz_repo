...
router.post("/", optionalAuth, async (req, res) => {
  ...
  const incidentSummary = incidents.slice(0, 6).map((a, i) =>
    `${i + 1}. [${a.severity}] ${a.title} — ${a.source || "unknown platform"}`
  ).join("\n");
...
Return ONLY valid JSON — no markdown fences:
{
  ...
  "triggers": ["<condition that triggers this runbook, max 3 — be specific>"],
  "steps": [
    {
      "step": <integer>,
...
Rules:
- Steps should be exhaustive — include every action needed to resolve the incident
- Commands must be exact and copy-paste ready
- Do not include placeholder text — be specific`;
