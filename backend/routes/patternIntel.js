...
router.post("/", optionalAuth, async (req, res) => {
  ...
  const incidentLog = analyses.slice(0, 40).map((a, i) =>
    `${i + 1}. [${a.severity}] "${a.title}" — platform: ${a.source || "unknown"} — date: ${a.created_at?.slice(0, 10) || "unknown"}`
  ).join("\n");
...
Return ONLY valid JSON — no markdown fences:
{
  ...
  "predictions": [
    "<proactive warning based on current patterns — max 2, phrased as 'If X continues, expect Y'>
  ],
...
- recommendations: be concrete — name specific actions, not vague advice`;
