...
const FEATURES = [
  { icon: <IconAnalyze/>, title: "AI-Powered Analysis", desc: "Paste any error log and receive an instant plain English diagnosis â root cause, severity score, and step-by-step resolution." },
  { icon: <IconTerminal/>, title: "Browser Terminal", desc: "Run diagnostic commands directly in your browser â ping, curl, kubectl, docker ps, netstat and more without leaving the platform." },
  ...
  { icon: <IconReport/>, title: "Downloadable Reports", desc: "Export a full formatted analysis report â severity, resolution steps, technical context, and prevention tips. Developer plan and above." },
...
// ââ Per-step chat âââââââââââââââââââââââââââââââââââââââââââââ
  ...
  return (
    <div className={styles.stepWrap}>
      ...
      <button className={`${styles.chatToggle} ${open ? styles.chatToggleOpen : ""}`} onClick={() => setOpen(o => !o)}>
        <IconChat/>
        {open ? "Hide chat" : msgs.length > 0 ? `${msgs.filter(m=>m.role==="assistant").length} replies Â· Ask more` : "Ask about this step"}
      </button>
      {open && (
        <div className={styles.chatBox}>
          {msgs.length === 0 && (
            <div className={styles.chatEmpty}>
              <div>Ask anything about this step â "What does this command do?", "I got a different error", "Is this safe to run in production?"</div>
          ...
          <div className={styles.chatInputWrap}>
            <input className={styles.chatInput} value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="Ask a follow-up questionâ¦" disabled={loading}/>
...
// ââ CursorDemo ââââââââââââââââââââââââââââââââââââââââââââââââ
const DEMO_LOG_SHORT = "ERROR: CrashLoopBackOff\nkubelet: Back-off restarting failed container\nExit code 1 â OOMKilled";
const GROUP_NAME     = "Production APIs";

const DEMO_PHASES = [
  { id:"idle",        scene:"analyzer",      cx:200, cy:120, dur:800,  label:"Opening SimpleLogzâ¦"              },
  { id:"typing",      scene:"analyzer",      cx:200, cy:120, dur:2200, label:"Pasting a K8s error logâ¦"         },
  { id:"click-btn",   scene:"analyzer",      cx:330, cy:215, dur:500,  label:"Running AI analysisâ¦",    clicking:true },
  { id:"analyzing",   scene:"analyzing",     cx:200, cy:165, dur:1600, label:"AI analyzing patternsâ¦"           },
  { id:"results",     scene:"results",       cx:155, cy:265, dur:1000, label:"Diagnosis: CrashLoopBackOff â"    },
  { id:"res-tab",     scene:"results",       cx:155, cy:265, dur:500,  label:"Viewing resolution stepsâ¦", clicking:true },
  { id:"res-steps",   scene:"results-steps", cx:200, cy:300, dur:1600, label:"Step-by-step fix plan"            },
  { id:"nav-proj",    scene:"results-steps", cx:22,  cy:120, dur:500,  label:"Going to Projectsâ¦",      clicking:true },
  { id:"projects",    scene:"projects",      cx:290, cy:52,  dur:1200, label:"Your project groups"              },
  { id:"new-grp-clk", scene:"projects",      cx:290, cy:52,  dur:500,  label:"Creating a new groupâ¦",   clicking:true },
  { id:"naming",      scene:"new-group",     cx:200, cy:165, dur:1800, label:"Creating 'Production APIs'â¦"      },
  { id:"grp-dash",    scene:"group-dash",    cx:200, cy:145, dur:1000, label:"Inside your project group"        },
  { id:"grp-log-clk", scene:"group-dash",    cx:72,  cy:230, dur:500,  label:"Opening Log Analyzerâ¦",   clicking:true },
  { id:"log-tab",     scene:"group-log",     cx:200, cy:200, dur:1300, label:"Analyze logs inside a group"      },
  { id:"code-clk",    scene:"group-log",     cx:160, cy:230, dur:500,  label:"Inspecting codeâ¦",        clicking:true },
  { id:"inspect",     scene:"group-code",    cx:200, cy:200, dur:1300, label:"Code Inspector â 3 issues found"  },
  { id:"runbook-clk", scene:"group-code",    cx:238, cy:230, dur:500,  label:"Opening Runbook Studioâ¦", clicking:true },
  { id:"runbook",     scene:"group-runbook", cx:200, cy:200, dur:1500, label:"AI-generated runbook ready"       },
  { id:"git-clk",     scene:"group-runbook", cx:312, cy:230, dur:500,  label:"Linking Git repoâ¦",       clicking:true },
  { id:"git",         scene:"group-git",     cx:200, cy:200, dur:1800, label:"GitHub repo connected â"          },
  { id:"done",        scene:"analyzer",      cx:200, cy:120, dur:700,  label:"And it starts againâ¦"            },
];
          ...
          <div style={{flex:1,overflow:"hidden",position:"relative",background:"#1a2438"}}>
            ...
            {scene==="analyzer" && (
              <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{background:"#1c2640",border:"1px solid rgba(79,142,247,.18)",borderRadius:10,overflow:"hidden"}}>
                  <div style={{padding:"6px 12px",background:"rgba(79,142,247,.06)",borderBottom:"1px solid rgba(79,142,247,.12)",fontSize:9,fontFamily:"var(--mono)",color:"rgba(255,255,255,.3)"}}>Auto-detect â¾</div>
                  <div style={{minHeight:90,padding:"10px 12px",fontFamily:"var(--mono)",fontSize:10,color:"rgba(255,255,255,.7)",lineHeight:1.6,whiteSpace:"pre-wrap"}}>
                    {typed || <span style={{color:"rgba(255,255,255,.06)",fontStyle:"italic"}}>Paste your error log hereâ¦</span>}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6,padding:"7px 12px",borderTop:"1px solid rgba(255,255,255,.05)",background:"rgba(255,255,255,.04)"}}>
                    ...
                    <div style={{marginLeft:"auto",padding:"5px 12px",borderRadius:8,background:"linear-gradient(135deg,#4f8ef7,#6366f1)",fontSize:9,fontWeight:700,color:"#fff",fontFamily:"var(--mono)",opacity:clicking?0.7:1,transform:clicking?"scale(0.95)":"scale(1)"}}>â Analyze</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {scene==="analyzing" && (
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:12}}>
                ...
                <div style={{fontSize:11,color:"rgba(255,255,255,.5)",fontFamily:"var(--mono)"}}>AI analyzing patternsâ¦</div>
                ...
                {["Production APIs","K8s Cluster","Auth Services"].map((name,i) => (
                  <div key={name} style={{padding:"8px 10px",marginBottom:6,borderRadius:8,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <div>
                      ...
                      <div style={{fontSize:8,color:"rgba(255,255,255,.25)",marginTop:1}}>{[4,2,6][i]} members Â· {[12,5,20][i]} analyses</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {scene==="group-dash" && (
              <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
                <div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
                  ...
                  <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:2}}>4 members Â· 12 analyses this week</div>
                </div>
            ...
            {scene==="group-log" && (
              <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
                ...
                <div style={{padding:"10px 14px",flex:1}}>
                  ...
                  <div style={{marginTop:8,padding:"6px 10px",background:"rgba(79,142,247,.08)",borderRadius:6,fontSize:8,color:"rgba(79,142,247,.9)",fontFamily:"var(--mono)"}}>â Analysis complete â CRITICAL severity</div>
                </div>
              </div>
            )}
              ...
              <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
                ...
                <div style={{padding:"10px 14px",flex:1}}>
                  <div style={{background:"#1c2640",border:"1px solid rgba(255,165,0,.2)",borderRadius:8,overflow:"hidden"}}>
                    <div style={{padding:"5px 10px",background:"rgba(255,165,0,.06)",borderBottom:"1px solid rgba(255,165,0,.15)",fontSize:8,color:"rgba(255,165,0,.7)",fontFamily:"var(--mono)"}}>â  3 issues found</div>
                    {[{line:14,msg:"Unhandled promise rejection"},{line:28,msg:"Memory leak â listener not removed"},{line:45,msg:"Deprecated: process.binding()"}].map((issue,i) => (
                  ...
                  <div style={{border:"1px solid rgba(99,102,241,.3)",borderRadius:8,padding:"10px 12px",background:"rgba(79,142,247,.06)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:8,color:"rgba(255,255,255,.3)"}}>main branch Â· last commit 2h ago</div>
                      </div>
                    </div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {["12 open PRs","3 failing checks","Last deploy: â"].map(badge => (
...
export default function Landing() {
  ...
  // ââ Guided hint ââââââââââââââââââââââââââââââââââââââââââââââ
  const [hintText,    setHintText]    = useState("");
  const [hintVisible, setHintVisible] = useState(true);

  useEffect(() => {
    if (log) return;
    const msg = "Paste your error message hereâ¦";
    let iv = null;
    let stopped = false;
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      setStatus("error");
      showToast(err.message?.includes("Daily limit") ? "Daily limit reached â upgrade for unlimited analyses" : (err.message || "Analysis failed"), "error");
    }
  }
  ...
  function exportReport() {
    ...
    const lines = [
      ...
      `\nââ RESOLUTION STEPS`, ...steps,
      `\nââ VERIFICATION`, ...(result.verification_commands||[]).map(c => `$ ${c}`),
      `\nââ TECHNICAL CONTEXT\n${result.technical_context}`,
      `\nââ PREVENTION`, ...(result.prevention||[]).map(p => `â¢ ${p}`),
      `\nââ ORIGINAL LOG\n${log}`,
      "\nâââââââââââââââââââââââââââââââââââââââ",
    ];
    ...
    <div className={styles.page}>
        ...
        <div className="container">
          <div className={styles.heroInner}>
            ...
            <p className={styles.sub}>
              Paste any error log. Get a plain English diagnosis and a step-by-step fix â instantly.
            </p>
          </div>
        </div>
      </section>

      {/* ââ Single-box Analyzer ââââââââââââââââââââââââââââââ */}
      <div className={styles.analyzerWrap}>
        <div className="container">
          ...
          {!isLoggedIn && (
            <div className={styles.usagePill}>
              2 free analyses daily Â· <Link to="/signup" style={{color:"var(--accent)"}}>Sign up for unlimited â</Link>
            </div>
          )}
          ...
          <div className={styles.analyzerFrame}>
            ...
            {/* Results â appear below input in the same box */}
            {(status === "done" || status === "error") && result && (
              <div className={styles.resultsSection}>
                ...
                <div style={{display:"flex",justifyContent:"flex-end",padding:"8px 16px",borderBottom:"1px solid var(--border)"}}>
                  {isPro
                    ? <button className="btn btn-ghost btn-sm" onClick={exportReport}>â Download Incident Report</button>
                    : <Link to="/pricing" className="btn btn-ghost btn-sm">â Download Incident Report</Link>
                  }
                </div>
                    ...
                    <div>
                      ...
                      {!isPro && (
                        <div className={styles.planGate}>
                          <div style={{fontWeight:600,marginBottom:6}}>ð Full CLI commands on Developer plan</div>
                          ...
                          <Link to="/pricing" className="btn btn-primary btn-sm">Upgrade â $10/mo</Link>
                        </div>
                      )}
                    </div>
                  )}
                      ...
                      {(result.related_errors||[]).length > 0 && (
                        <>
                          ...
                          <ul style={{listStyle:"none",display:"flex",flexDirection:"column",gap:6,marginTop:8}}>
                            {result.related_errors.map((e,i) => <li key={i} style={{fontFamily:"var(--mono)",fontSize:12,color:"var(--t2)",paddingLeft:14}}>â {e}</li>)}
                          </ul>
                        </>
                      )}
                    </div>
                    ...
                    <ul style={{listStyle:"none",display:"flex",flexDirection:"column",gap:12}}>
                      {(result.prevention||[]).map((p,i) => (
                        <li key={i} style={{display:"flex",gap:10,fontSize:14}}>
                          <span style={{color:"var(--green)",fontWeight:700,flexShrink:0}}>â</span>{p}
                        </li>
                      ))}
                    </ul>
                  )}
      ...
      {/* ââ Cursor Demo ââââââââââââââââââââââââââââââââââââââââââ */}
      <CursorDemo />

      {/* ââ CTA + Footer âââââââââââââââââââââââââââââââââââââââââ */}
      <div className="container">
        <div className={styles.ctaSection}>
          <div className={styles.ctaInner}>
            ...
            <p className={styles.ctaSub}>Paste any error log. Get a plain English diagnosis and a step-by-step fix â instantly.</p>
      ...
      {/* ââ Testimonials carousel âââââââââââââââââââââââââââââââ */}
      <TestimonialCarousel/>

      {/* ââ Share your experience âââââââââââââââââââââââââââââââ */}
      <div className="container">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16, position:"relative" }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, color:"var(--t3)", marginTop:3, lineHeight:1.5 }}>
                Share a quick review â it helps other engineers discover us.
              </div>
            </div>
          </div>
