...
const MAX_PER_DAY = 2;
// First popup: 90 seconds after load. Second: 6 hours later (if same session).
const FIRST_DELAY_MS  = 90_000;  // 90 s
const MIN_INTERVAL_MS = 6 * 60 * 60 * 1000; // 6 h between shows

function getTodayKey() {
  return new Date().toISOString().slice(0, 10); // "2025-06-11"
}
...
const RATINGS = [
  { emoji: "ð£", label: "Very bad"  },
  { emoji: "ð", label: "Bad"       },
  { emoji: "ð", label: "Okay"      },
  { emoji: "ð", label: "Good"      },
  { emoji: "ð", label: "Excellent" },
];
...
export default function FeedbackPopup() {
  ...
  async function submit() {
    ...
    try {
      ...
      // fail silently â user already sees thank-you
    }
      ...
      {sent ? (
        ...
        <>
          ...
          <div style={{ fontSize: 12, color: "var(--t3)", marginBottom: 16 }}>
            Takes 5 seconds â your feedback helps us improve.
          </div>
