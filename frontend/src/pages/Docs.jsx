      ...
      <main style={{ flex:1, paddingTop:52, paddingBottom:120, paddingLeft:52, minWidth:0 }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize:14, color:"var(--t2)", lineHeight:1.75, maxWidth:540, marginBottom:20 }}>
            Everything you need to debug faster — from pasting your first log to running full incident runbooks across your engineering groups.
          </p>
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <p style={{ fontSize:14, color:"var(--t2)", lineHeight:1.8, marginBottom:20 }}>
            SimpleLogz is an AI-powered developer platform that transforms raw error logs, stack traces, and application errors into instant root-cause diagnoses and actionable fixes. Whether you're debugging a production outage or reviewing code for security vulnerabilities, SimpleLogz returns answers in seconds — not hours.
          </p>
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <Callout type="warning">
            To create Groups, inspect code, generate runbooks, and run unlimited analyses, you need a <strong>Developer plan</strong> ($5/month or $49/year). <Link to="/pricing" style={{color:"#6c5ce7", fontWeight:700}}>See pricing →</Link>
          </Callout>
          <StepItem n={1} title="Paste your error log">
            Navigate to the <Link to="/" style={{color:"#6c5ce7", fontWeight:600}}>Analyzer</Link> on the homepage. Paste any error log — Lambda timeouts, database errors, stack traces, HTTP failures, anything.
          </StepItem>
          <StepItem n={2} title="Click Analyze">
            Hit <strong style={{color:"var(--t1)"}}>→ Analyze</strong> or press <kbd style={{ background:"var(--bg3)", border:"1px solid var(--border)", borderRadius:5, padding:"2px 7px", fontSize:11, fontFamily:"var(--mono)" }}>Ctrl+Enter</kbd>. The AI reads your log and returns a structured diagnosis.
          </StepItem>
          ...
          <Callout type="tip">
            Try this example log — paste it into the analyzer to see a full diagnosis instantly.
          </Callout>
          <CodeBlock label="Example — AWS Lambda DB timeout">
...
RequestId: 8f3d2c1a-e4b9-11ed-a05b Version: $LATEST`}
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20 }}>
            {[
              { icon:Icons.Activity,  title:"Severity rating",     desc:"CRITICAL, HIGH, MEDIUM, or LOW — so you know what to fix first." },
              ...
              { icon:Icons.Globe,     title:"Source detection",    desc:"Auto-detects the platform — AWS, Node.js, Postgres, Docker, and more." },
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <p style={{ fontSize:14, color:"var(--t2)", lineHeight:1.8, marginBottom:20 }}>
            Paste any code snippet and the Code Inspector scans it for bugs, security vulnerabilities, anti-patterns, and performance issues — then suggests a fix for each one.
          </p>
          ...
          <Callout type="info">
            Each detected issue includes a <strong>Fix Now</strong> button. Clicking it sends the issue to Claude AI and applies the corrected code directly — no copy-pasting needed.
          </Callout>
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <StepItem n={4} title="Save for future reference">Completed runbooks are saved to the group — your team can reference past responses to avoid repeating the same debugging process.</StepItem>
          <Callout type="tip">
            Create one group per service. That way, each service accumulates its own library of incident runbooks over time — a living knowledge base for your engineering team.
          </Callout>
        </section>
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <p style={{ fontSize:14, color:"var(--t2)", lineHeight:1.8, marginBottom:20 }}>
            Pattern Intelligence scans your logs over time to identify recurring issues, error trends, and systemic problems — before they become outages.
          </p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            ...
            <FeatureCard icon={Icons.Cpu}        title="Health scoring"      desc="Each group receives a health score (0–100) based on recent error patterns and frequency." accent="#a29bfe"/>
          </div>
        </section>
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <p style={{ fontSize:14, color:"var(--t2)", lineHeight:1.8, marginBottom:20 }}>
            Groups are shared engineering workspaces organised by service. Each group gets its own Log Analyzer, Code Inspector, Runbook Studio, and Pattern Intelligence — keeping everything scoped by service.
          </p>
          <StepItem n={1} title='Navigate to Groups'>Click <strong style={{color:"var(--t1)"}}>Groups</strong> in the sidebar.</StepItem>
          <StepItem n={2} title='Create a new group'>Click <strong style={{color:"var(--t1)"}}>+ New Group</strong> and name it after the service — e.g. "Production API", "Auth Service", "Worker Queue".</StepItem>
          <StepItem n={3} title="Start working">Paste logs, inspect code, generate runbooks — all scoped to that group's history and context.</StepItem>
          <Callout type="warning">
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <Callout type="warning">
            Free users can only analyze error logs — limited to <strong>2 analyses per day</strong>. Creating Groups, running Code Inspections, generating Runbooks, and accessing Pattern Intelligence all require a <strong>Developer plan</strong>.
          </Callout>
            ...
            <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:14, padding:"24px" }}>
              ...
              {["2 error log analyses per day","Log Analyzer only","Community forum access","No Groups or Projects","No Code Inspector","No Runbook Studio"].map((f, i) => (
                <div key={f} style={{ display:"flex", alignItems:"center", gap:9, padding:"6px 0", fontSize:13, color: i > 1 ? "var(--t3)" : "var(--t2)" }}>
                  <span style={{color: i > 1 ? "var(--t3)" : "#34d399", flexShrink:0}}>{i > 1 ? "✕" : <Icons.CheckCircle/>}</span> {f}
                </div>
              ))}
            </div>
            {/* Developer */}
        ...
        <section style={{ marginBottom:60 }}>
          ...
          <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:14, padding:"0 24px" }}>
            {[
              ...
              { q:"What types of logs does SimpleLogz support?", a:"Any plain-text error log — stack traces, Lambda timeouts, database errors, HTTP errors, container logs, CI/CD failures, and more. The AI auto-detects the platform and adjusts its analysis accordingly." },
              ...
              { q:"Can I cancel my Developer plan anytime?", a:"Yes. Cancel anytime from Settings → Billing. You'll retain full access until the end of your current billing period with no penalties." },
              { q:"What's the benefit of the annual plan?", a:"The Developer annual plan is $49/year — equivalent to $4.08/month — saving you $11 compared to paying monthly. You get the same features with two months effectively free." },
